...
public class CargaDescargaScreen extends javax.swing.JFrame {
    ...
    public final void leituraArquivo(File arquivo) {
        FileReader fr = null;
        try {
    ...
    public void escreverArquivo(File arquivo, ArrayList<Transportadora> lista) throws IOException {
        boolean existe = arquivo.exists();
        if (existe) {
            arquivo.delete();
        }
        ...
        for (Transportadora frete : lista) {
    ...
    public void escreverArquivo(File arquivo, Stack<Transportadora> lista) throws IOException {
        boolean existe = arquivo.exists();
        if (existe) {
            arquivo.delete();
        }
    ...
    public CargaDescargaScreen() {
        initComponents();
        leituraArquivo(cargasRecebidas);
        leituraArquivo(caminhao);
    }
    ...
    private void carregarCaminhaoButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_carregarCaminhaoButtonMouseClicked
        ...
        if(fretesCarregados.isEmpty()){
            ...
            JOptionPane.showInternalMessageDialog(rootPane, "Caminhão ja carregado, deve descarregar o caminhão primeiro");
        }
        ...
        try {
            escreverArquivo(cargasRecebidas, listaFretes);
            escreverArquivo(caminhao, fretesCarregados);
        } catch (IOException ex) {
